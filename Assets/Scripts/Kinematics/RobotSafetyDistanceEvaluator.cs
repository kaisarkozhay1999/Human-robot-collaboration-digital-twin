...
namespace DigitalTwin.Kinematics
{
    ...
    public sealed class RobotSafetyDistanceEvaluator : MonoBehaviour
    {
        ...
        [Tooltip("Ignore disabled/inactive PoseBoneDriver instances discovered through Resources.FindObjectsOfTypeAll.")]
        public bool ignoreInactivePoseDrivers = true;
        [Tooltip("Ignore PoseBoneDriver joint positions once their last packet is older than staleAfterSeconds.")]
        public bool ignoreStalePoseDriverJoints = true;
        ...
        public float warningThresholdMeters = 0.32f;
        public float stopThresholdMeters = 0.25f;
        public float releaseThresholdMeters = 0.30f;

        [Header("Gesture state")]
        public bool readGestureReceiver = true;
        [Tooltip("Ignore StopGoReceiver.currentCommand after this many seconds so stale gesture commands do not override proximity decisions. Set to 0 or less to keep the old latched behavior.")]
        public float gestureCommandFreshSeconds = 2.0f;
        ...
        public bool publishSafetyCommands = true;
        ...
        public int ActiveHumanJointCount => CurrentHumanJointPositions == null ? 0 : CurrentHumanJointPositions.Length;
        public float HumanPoseAgeSeconds => float.IsNegativeInfinity(LastHumanPoseTime) ? float.PositiveInfinity : Time.time - LastHumanPoseTime;
        ...
        private void RefreshLiveSourcesIfNeeded()
        {
            ...
            if (readPoseBoneDrivers && autoRefreshPoseBoneDrivers)
                poseBoneDrivers = FindScenePoseBoneDrivers(ignoreInactivePoseDrivers);
            else if (readPoseBoneDrivers && (poseBoneDrivers == null || poseBoneDrivers.Length == 0))
                poseBoneDrivers = FindScenePoseBoneDrivers(ignoreInactivePoseDrivers);
        }

        private static global::PoseBoneDriver[] FindScenePoseBoneDrivers(bool activeOnly)
        {
            ...
            foreach (global::PoseBoneDriver driver in drivers)
            {
                ...
                if (activeOnly && !driver.isActiveAndEnabled)
                    continue;
        ...
        private void ApplyLiveGestureState()
        {
            ...
            if (gestureCommandFreshSeconds > 0f &&
                (float.IsInfinity(gestureReceiver.CommandAgeSeconds) ||
                 gestureReceiver.CommandAgeSeconds > gestureCommandFreshSeconds))
            {
                gestureStopRequested = false;
                gestureGoRequested = false;
                return;
            }
        ...
        private Vector3[] ReadHumanJointPositions(float nowSeconds)
        {
            ...
            if (readPoseBoneDrivers && poseBoneDrivers != null)
            {
                foreach (global::PoseBoneDriver driver in poseBoneDrivers)
                {
                    ...
                    if (ignoreInactivePoseDrivers && !driver.isActiveAndEnabled)
                        continue;

                    if (ignoreStalePoseDriverJoints &&
                        (float.IsNegativeInfinity(driver.LastPoseUpdateTime) ||
                         nowSeconds - driver.LastPoseUpdateTime > staleAfterSeconds))
                    {
                        continue;
                    }
